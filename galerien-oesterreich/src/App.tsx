...
export default function GalleryDashboard() {
  ...
  const exportCsv = () => {
    ...
    const header = ["Bundesland", "Stadt", "Name", "Kategorie", "Adresse", "E-Mail", "Weitere E-Mail", "Ansprechperson", "Funktion", "Anrede", "Telefon", "Website", "Status", "Vertrauen"];
    const rows = visibleGalleries.map(gallery => [gallery.state ?? "", gallery.city, gallery.name, gallery.category, gallery.address, gallery.email ?? "", gallery.moreEmail ?? "", gallery.contacts ?? "", gallery.role ?? "", gallery.salutation ?? "", gallery.phone ?? "", gallery.website ?? "", gallery.status, gallery.confidence]);
    const csv = [header, ...rows].map(row => row.map(csvField).join(";")).join("\r\n");
    const blob = new Blob(["﻿", csv], { type: "text/csv;charset=utf-8;" });
    const url = URL.createObjectURL(blob);
    const link = document.createElement("a");
    link.href = url;
    link.download = `gvoe-export-${new Date().toISOString().slice(0, 10)}.csv`;
