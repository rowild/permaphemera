...
- `pp_exhibitions.json` → `"tour": "/media/tours/<tour id>/" | null` (leading and trailing slash), typed in `app/types/content.ts`, passed through `resolveExhibitions.ts` as `tour?: string`. `null` keeps the button disabled with the "in preparation" line.
