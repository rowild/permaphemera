...
`app/pages/exhibitions/index.vue` lists every routed local exhibition by start date, newest first, and preserves that chronology while searching across title, artist, gallery, city, date, medium, and record text with `q` in the URL. Featured status changes presentation only and never moves a record out of chronological order. `app/pages/exhibitions/[slug].vue` resolves the same data into individual records with dates, opening hours, vernissage, source PDF, gallery-aware breadcrumbs and return paths, related exhibitions, and a prepared—but disabled—360-degree experience section.
